...
2. エージェントが複数ある場合は `config.toml` で `openclaw_agent` を指定:

```toml
openclaw_agent = "my-agent-name"
```

エージェントが 1 つだけの場合は空のままで自動選択されます。
...
チャット機能で使用するエージェント名を指定できます。省略時は OpenClaw のデフォルトエージェントが使われます。
