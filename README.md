...
**doll** は、[OpenClaw](https://github.com/openclaw/openclaw) AI エージェントと連動するデスクトップマスコットです。エージェントが回答するたびに、その感情に応じてキャラクターの表情が変わります。ローカル CLI 経由でもリモートサーバー経由でも動作します。
...
- **チャット機能**: マスコットをクリックして直接 OpenClaw エージェントにメッセージを送信
...
### リモートモード (別端末の OpenClaw に接続)

同一 LAN 上の別マシンで動いている OpenClaw Gateway に接続して使うことができます。

**doll 側 (`~/.config/doll/config.toml`):**
...
- `url` — Gateway の HTTP アドレス。設定すると doll はリモートモードになり、`POST /v1/responses` でメッセージを送信します。
- `token` — Gateway の認証トークン (サーバー側の `OPENCLAW_GATEWAY_TOKEN` と一致させる)。
- HTTP サーバーが `0.0.0.0` にバインドされ、LAN 上の OpenClaw からステータス通知を受信できるようになります。

**OpenClaw (サーバー) 側:**

1. Gateway の `/v1/responses` エンドポイントを有効にする
2. Skill / Hook が doll に到達できるよう、環境変数を設定する:
   ```bash
   export DOLL_HOST=192.168.1.50   # doll マシンの LAN IP
   export DOLL_PORT=3000            # デフォルトなら省略可
   ```

### チャット機能 (実験的)

マスコットをクリックするとテキスト入力欄が開き、OpenClaw エージェントに直接メッセージを送信できます。送信したメッセージとエージェントの応答はマスコット下部にチャットログとして表示されます。

**ローカルモード** (デフォルト) では `openclaw` CLI を直接起動します。**リモートモード** では Gateway の HTTP API を使用します。

ローカルモードでこの機能を使うには:

1. OpenClaw CLI (`openclaw`) にパスが通っていること
2. エージェントが複数ある場合は `config.toml` の `[openclaw]` セクションでエージェント名を指定

エージェントが 1 つだけの場合は未指定で自動選択されます。設定項目の詳細は `config.toml` 内のコメントを参照してください。
...
`[openclaw]` セクションでエージェント名やリモート接続先を設定します。詳しくは `config.toml` 内のコメントおよび上記「リモートモード」セクションを参照してください。

### VoiSona Talk TTS

`config.toml` の `[voisona]` セクションで `enabled = true` にし、認証情報を設定してください。設定可能な項目は `config.toml` 内のコメントを参照してください。
